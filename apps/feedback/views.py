...
def sconsolidated(request, username):
	"""
	This view displays the student feedback reports of the faculty.
	"""
...
class Student_Report(TemplateView):
	...
	def get_context_data(self, username, *args, **kwargs):
		...
		for form in forms:
			...
			results[form] = answers
		context["results"] = results
		context["process"] = current_process
		return context
...
class select_report(FormView):
	...
	def post(self, request, *args, **kwargs):
		...
		return redirect(
			reverse_lazy("previous_reports", kwargs={"id": process_id})
		)

@login_required(login_url="/signin/")
def previous_consolidated(request, id):
	"""
